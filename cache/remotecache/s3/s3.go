...
func ResolveCacheExporterFunc() remotecache.ResolveCacheExporterFunc {
	return func(ctx context.Context, g session.Group, attrs map[string]string) (remotecache.Exporter, error) {
		...
		mc, err := newMinioClient(ctx, config)
		if err != nil {
			return nil, err
		}
...
func newMinioClient(ctx context.Context, cfg Config) (*minioClient, error) {
	...
	log.Printf("URL: %v", parsedUrl.Host)
	log.Printf("Scheme: %v", parsedUrl.Scheme)
	log.Printf("Style: %v", bucketLookup)
	log.Printf("Access Key: %v", cfg.AccessKeyID)
	log.Printf("Secret Key: %v", cfg.SecretAccessKey)
...
func (m *minioClient) exists(ctx context.Context, key string) (*time.Time, *int64, error) {
...
func buildCopySourceRange(start int64, objectSize int64) (int64, int64) {
...
func (m *minioClient) touch(ctx context.Context, key string, size *int64) error {
